...
<div className="w-full overflow-x-auto">
  ...
  <div className="inline-flex w-max min-w-full gap-2 sm:gap-4 justify-center bg-white shadow rounded-2xl px-2 sm:px-4 py-2 mb-8 border border-[#e9d4ff]">
    {TABS.map((tab) => (
      <button
        key={tab.label}
        onClick={() => setActiveTab(tab.label)}
        className={`px-4 sm:px-6 py-2 rounded-xl transition-all duration-200 font-medium text-sm sm:text-base whitespace-nowrap
          ${
            activeTab === tab.label
              ? 'bg-[#6750a4] text-white shadow font-semibold scale-105'
              : 'bg-[#f3e6f1] text-[#6750a4] hover:bg-[#e9d4ff]'
          }
        `}
        style={{
          flexShrink: 0,
      ...
      >
        {tab.label}
          ...
          {/* Rendered Content */}
          <div className="w-full bg-white rounded-2xl shadow p-4 sm:p-8 min-h-[400px]">
            {renderContent()}
          </div>
        </div>
      </div>
