...
const Activate = () => {
  ...
  const handleActivate = async (proposalId) => {
    ...
    try {
      await api.put(`/college/proposal/${proposalId}`, { status: 'active' });
      fetchFiles(); 
      fetchProposals(); // Refresh proposals to update status
    } catch (err) {
      alert('Failed to activate proposal. Please try again.');
    } finally {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-8">
      <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-8 shadow-lg">
        <h3 className="text-2xl font-bold mb-6 text-[#4f378a] text-center">Proposal Activation</h3>
  ...
  <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getStatusClass(file.status)}`}>
    {file.status}
                    ...
                    <td className="px-4 py-2 border-b text-center">
                      <button
                        onClick={() => handleActivate(file.proposal_id)}
                        disabled={file.status === 'active' || activatingId === file.proposal_id}
                        className={`px-5 py-1.5 rounded font-semibold text-xs transition-all ${
                          file.status === 'active'
                            ? 'bg-green-200 text-green-800 cursor-not-allowed'
                            : activatingId === file.proposal_id
