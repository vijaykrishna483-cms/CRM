...
const DocxForm = () => {
  ...
  const handleEmployeeSelect = (e) => {
    ...
    const selectedEmployee = employeeData.find(emp => emp.employee_id === employeeId);

    if (selectedEmployee) {
      const salary = parseFloat(selectedEmployee.salary) || 0;
      // Split address into up to 3 lines
      const addressParts = (selectedEmployee.address || '').split(',').map(s => s.trim());
  ...
  // Combine address lines for submission
  const getFullAddress = () => {
    return [formData.address1, formData.address2, formData.address3]
      .filter(Boolean)
      .join(', ');
  };
  ...
  <div className='fixed w-[100vw] z-[100]'>
        <Navbar/>
        </div>         <div className="max-w-3xl mx-auto  px-2">
        <div className=" p-1 md:p-2 relative overflow-hidden">
          ...
          <div className="relative z-10  p-6 ">
            <h1 className="text-3xl text-[#88139a] font-bold text-center tracking-wide">Invoice Generator</h1>
            <p className="text-center text-[#88139a] mt-2">Create professional invoices in seconds</p>
          </div>
          {/* Form Section */}
          <div className="relative z-10 p-6">
  ...
  {/* Employee Select */}
  <div>
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Select Employee</label>
    <select
      value={employeeData.find(emp => emp.employee_name === formData.employeeName)?.employee_id || ''}
  ...
  {/* Designation */}
  <div>
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Designation</label>
    <input
      name="designation"
  ...
  {/* Salary */}
  <div>
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Salary (₹)</label>
    <input
      name="salary"
      placeholder="Salary"
  ...
  {/* Month */}
  <div>
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Month</label>
    <input
      name="month"
      placeholder="Month"
  ...
  {/* PAN ID */}
  <div>
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">PAN ID</label>
    <input
      name="panId"
      placeholder="PAN ID"
  ...
  <div className="md:col-span-2">
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Address Line 1</label>
    <input
      name="address1"
  ...
  {/* Address Line 2 */}
  <div className="md:col-span-2">
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Address Line 2</label>
    <input
      name="address2"
  ...
  {/* Address Line 3 */}
  <div className="md:col-span-2">
    <label className="block text-sm font-semibold text-[#4f378a] mb-1">Address Line 3</label>
    <input
      name="address3"
...
{/* Amount in Words */}
<div className="mt-6">
  <label className="block text-sm font-semibold text-[#4f378a] mb-1">Amount in Words</label>
  <input
    name="amountInWords"
  ...
  >
    Generate Invoice
          ...
          <div className="relative z-10 bg-gray-50 p-4 text-center text-sm text-gray-500 border-t rounded-b-2xl">
            Your invoice will be downloaded as a Word document
          </div>
        </div>
      </div>
      <Footer/>
